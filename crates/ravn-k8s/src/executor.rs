...
use kube::{Api, Client};
use ravn_actuator::{CapabilityExecutor, ExecError, ExecFuture};
...
impl CapabilityExecutor for KubeExecutor {
    fn run<'a>(&'a self, cap: &'a Capability) -> ExecFuture<'a> {
        // The K8s API calls are natively async (#144 made the executor trait
        // async), so we await them directly — no `block_on` runtime re-entry.
        Box::pin(async move {
            match cap {
                Capability::DeletePod { namespace, pod } => {
                    self.delete_pod(namespace, pod).await
                }
                Capability::RestartDeployment { namespace, deployment } => {
                    self.restart_deployment(namespace, deployment).await
                }
                Capability::PodState { namespace, pod_prefix } => {
                    self.pod_state(namespace, pod_prefix).await
                }
