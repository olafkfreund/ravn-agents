    ...
    E: CapabilityExecutor + Send + Sync + 'static,
{
    ...
    while let Some(msg) = sub.next().await {
        ...
        // It is sync (mirrors the host actuator contract), so we spawn_blocking.
        let executor_ref = Arc::clone(&executor);
        let key_ref = verifying_key.clone();
        let env_ref = env.clone();
        let result = tokio::task::spawn_blocking(move || {
            handle_command(executor_ref.as_ref(), &key_ref, &env_ref)
        })
        .await
        .context("handle_command panicked")?;
