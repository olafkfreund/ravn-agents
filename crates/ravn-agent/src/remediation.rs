...
/// The pull loop: every `poll_secs` (minimum [`MIN_POLL_SECS`]), refresh the
/// trusted keyring (so a key rotation reaches the fleet on the next check-in,
/// #150), then fetch pending commands and process them.
///
...
///
/// `ravnd` verifies only for its own dispatch decision; the actuator
/// independently re-verifies, so a momentarily stale keyring is never unsafe.
...
pub async fn run(
    ...
    loop {
        ...
        // pinned now, before any command signed by the new key arrives (#150).
        refresh_keyring(&http, &base_url, api_token.as_deref(), &ring, &cred_dir).await;

        match poll_once(&http, &base_url, agent_id, api_token.as_deref(), &ring, &ledger, &socket)
            .await
        {
